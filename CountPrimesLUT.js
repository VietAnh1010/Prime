...
const TinyPhiLUT = function() {
	const arr = new Uint16Array(TinyPhiOddCirc);  
	arr.fill(1);
	for (const p of TinyPhiPrimes) {
		if (p <= 2) 
			continue;
		arr[p >> 1] = 0;
		for (let c = (p * p) >> 1; c < TinyPhiOddCirc; c += p) 
			arr[c] = 0; 
	}
	for (let i = 0, acc = 0; i < TinyPhiOddCirc; ++i) {
		acc += arr[i]; 
		arr[i] = acc; 
	}
	return arr; 
...
function tinyPhi(x) {
	...
	const rem = (ndx - numtots * TinyPhiOddCirc);
...
function countPrimesTo(lmt) {
	...
	const oprms = function() {
		...
		for (let i = 0; i <= mxndx; ++i) 
			arr[i] = (i + i + 3) >>> 0;
		let bp = 3 >>> 0;
		while (true) {
			let i = (bp - 3) >>> 1; 
			let sqri = ((i + i) * (i + 3) + 3) >>> 0;
			if (sqri > mxndx) 
				break;
			if (arr[i] != 0) 
...
console.log(countPrimesTo(10 ** 9))
